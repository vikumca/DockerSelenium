...
public class RallyIntegration {
    ...
    public static void updatedTestCaseStatusInRally(String tcID,int resultStatus,String buildNumber, String notes) throws Exception{
        try{
            ...
            RallyRestApi restApi = new RallyRestApi(new URI("https://rally1.rallydev.com"), "dikshitha.sheth@carrier.com","Dikshu91#*");
            restApi.setProxy(new URI("http://zs-servproxy.utc.com"), "vikas.sharma@carrier.com", "Testings@1");
            ...
            if(createResponse.wasSuccessful()){
                Logs.info("TestCase with Id:"+tcID+" is updated successfully in Rally");
            }
            else {
                Logs.error("Failed to update the TestCase status with Id:"+tcID+" in Rally");
            }
            restApi.close();
        }

        catch (IOException e){
            Logs.error("Authentication error --"+e);
        }
        catch (URISyntaxException e) {
            Logs.error("Exception occurred in method updatedTestCaseStatusInRally "+e);
        }

        catch (Exception e) {
            Logs.error("Rally Updates issue --" + e);
        }
    }
}
